...
fn check(ctx: Arc<Context>) -> Result<()> {
    ...
    assert_eq!(user_data_types.len(), 4, "Found {} user data types, expected 4", user_data_types.len());
    let controllers = ctx.all_controllers().collect::<Vec<_>>();
    assert_eq!(controllers.len(), 1, "Found {} controllers, expected 1", controllers.len());
    ...
    let found = ctx.all_controllers().find(|(deleter_id, _)| {
        ...
        user_data_types.iter().all(|&t| {
            ...
            let flows = ctx.any_flows(&sources, &delete_sinks, paralegal_policy::EdgeType::Data)
                .is_some();
            if !flows {
                println!("User data not deleted: {}", ctx.describe_def(t));
            }
            flows
        })
    });
